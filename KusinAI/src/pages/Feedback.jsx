...
function Feedback() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showToast({ message: "Feedback sent successfully", type: "success" });
      setForm({ name: "", email: "", message: "" });
    } catch (err) {
      setSuccess("");
      setErrors({ submit: "Failed to send feedback. Please try again later." });
      showToast({ message: "Failed to send feedback", type: "error" });
    }
    setLoading(false);
  };
  ...
  return (
    <PageTransition>
      <MainLayout>
        <main className="flex-grow flex items-center justify-center text-text text-center px-6 py-8">
          <form
            className="bg-surface/90 backdrop-blur-sm p-6 rounded-lg max-w-xl w-full text-left space-y-4 shadow-xl"
